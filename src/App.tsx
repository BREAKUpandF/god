...
const PhotoOrnaments = ({ state }: { state: 'CHAOS' | 'FORMED' }) => {
  ...
  const data = useMemo(() => {
    return new Array(count).fill(0).map((_, i) => {
      ...
      if (layerRatio < 0.5) {
        y = -h / 2 + Math.random() * (h * 0.4);
      } else {
        y = -h / 2 + (h * 0.4) + Math.random() * (h * 0.6);
      }

      // 修复径向分布 - 让树顶照片更贴近树干  
      const normalizedY = (y + (h / 2)) / h;
      const minRadius = 0.2 + normalizedY * 0.3; // 树顶最小半径更小  
      const currentRadius = (rBase * (1 - normalizedY * 0.7)) + minRadius;
...
const Meteors = () => {
  ...
  const meteors = useMemo(() => {
    return new Array(count).fill(0).map(() => ({
      ...
      startPhi: Math.random() * Math.PI * 0.5, // 只在上半球  
  ...
  useFrame((state) => {
    ...
    groupRef.current.children.forEach((child, i) => {
      ...
      // 计算流星位置  
      const adjustedTime = (time + meteor.delay) % 20;
      if (adjustedTime < 5) {
        // 流星活跃期  
        const progress = adjustedTime / 5;
        const radius = 80;

        // 起始位置  
        const startX = radius * Math.sin(meteor.startPhi) * Math.cos(meteor.startTheta);
        const startY = radius * Math.cos(meteor.startPhi);
        const startZ = radius * Math.sin(meteor.startPhi) * Math.sin(meteor.startTheta);

        // 结束位置（向下移动）  
        const endX = startX + meteor.length * Math.cos(meteor.startTheta);
        const endY = startY - meteor.length;
        const endZ = startZ + meteor.length * Math.sin(meteor.startTheta);
        ...
        material.opacity = meteor.opacity * (1 - progress * 0.8);
        mesh.visible = true;
      } else {
        mesh.visible = false;
      }
  ...
  return (
    <group ref={groupRef}>
      {meteors.map((meteor, i) => (
        <mesh key={i}>
          <cylinderGeometry args={[0.05, 0.1, meteor.length, 8]} />
          <meshBasicMaterial
            color="#ffffff"
            transparent
        ...
        if (videoRef.current.videoWidth > 0) {
          ...
          if (results.gestures.length > 0) {
            ...
            if (results.landmarks.length > 0) {
              const speed = (0.5 - results.landmarks[0][0].x) * 4.0;
  ...
  useEffect(() => {
    const initAudio = async () => {
      ...
      try {
        ...
        console.log('Audio autoplay was blocked, waiting for user interaction to play');
      }
    };
